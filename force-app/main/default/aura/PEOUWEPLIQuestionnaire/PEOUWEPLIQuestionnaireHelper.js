({
    ...
    clearOutFields : function(component, event, helper) {
        ...
        if(component.get('v.PEOChecklist.has_150_or_more_emp_for_svc_agreement__c') != 'Yes' && component.get('v.PEOChecklist.has_three_or_more_employment_matters__c') != 'Yes' && component.get('v.PEOChecklist.has_any_emp_matter_exceeded_75k_inclusve__c') != 'Yes')
        {
            ...
            relatedFieldList.push('num_emp_matters_3_past_yrs_ignore_cost__c');
        }
        ...
        relatedFieldList.push('prev_12_mnth_rif_exp_and_emp_count__c','future_12_mnth_rif_exp_emp_count_nd_date__c','desc_circ_potential_lead_to_matter__c');
    ...
    setAkn: function(component, event, helper) {	
        ...
        var matterFidIdverified = '';
        epliFieldsMissing = this.verifyEPLIFields(component, event);
        policyPeriodVerified = this.verifyPolicyPeriods(component, event);
        matterFidIdverified = this.verifyMatterFedIdFields(component, event, helper);
        if(epliFieldsMissing == '' && policyPeriodVerified == true && matterFidIdverified == '') { 
            ...
            if(matterFidIdverified.length != 0) {
                epliFieldsMissing += matterFidIdverified;
            }
            component.set("v.agreementSigned", false);
    ...
    eraseMatter: function(cmp, e, helper) {
        ...
        if (n === 6) q.Additional_Matters_Information__c = '';
            ...
            let federalAPI = `Matter${n}_Fed_id__c`;
            console.log(matterAPI)
            q[matterAPI] = '';
            q[federalAPI] = '';
            relatedFieldList.push(''+matterAPI,''+federalAPI);
        }
    ...
    saveProgress : function(component, event, helper) {
        return new Promise(function(resolve, reject) {
            ...
            if(isCommUser){
                ...
                relatedFieldList.push('Peo_EPLI_formStatus__c','Peo_EPLI_SubmissionTime__c');
                helper.relatedFieldChanges(component, event, helper,'PEO_Onboarding_Checklist__c',relatedFieldList);
            }
            ...
            if(relatedFieldListToSave.length>0){
                ...
                relatedFieldListToSave.forEach(function (item, index) {
                    ...
                    if(objectAPINameToSave == 'PEO_Onboarding_Checklist__c'){
                        ...
                        recordId = cmp.get('v.parentAccount');
                    }
    ...
    verifyMatterFedIdFields : function(component, event, helper) {
        ...
        component.find("EPLIRequiredField").forEach( function(itemcmp) {
            if(itemcmp != 'undefined') {
                let matterFedField = (itemcmp.get('v.name') == 'PEO_Onboarding_Checklist__c.Matter1_Fed_id__c'  ||
                                      itemcmp.get('v.name') == 'PEO_Onboarding_Checklist__c.Matter2_Fed_id__c' ||
                                      itemcmp.get('v.name') == 'PEO_Onboarding_Checklist__c.Matter3_Fed_id__c' ||
                                      itemcmp.get('v.name') == 'PEO_Onboarding_Checklist__c.Matter4_Fed_id__c' || 
                                      itemcmp.get('v.name') == 'PEO_Onboarding_Checklist__c.Matter5_Fed_id__c'
                                     );
                if(!itemcmp.get("v.disabled") && itemcmp.get("v.required")) {
                    let val = ''+itemcmp.get("v.value");
                    if(matterFedField && val.length && val.length < 9) {
                        missingFieldList += itemcmp.get('v.label')+'\n';
                    }
                    else {
                        console.log(' verifyMatterFedIdFields if loop itemcmp.get("v.label") = '+ itemcmp.get("v.label"));
                        console.log(' verifyMatterFedIdFields if loop itemcmp.get("v.value") = '+ itemcmp.get("v.value"));
                    }
                }
                else{
                    if(matterFedField && !itemcmp.get("v.required")){
                        let matterFedFieldValue = itemcmp.get("v.value");
                        if(typeof(matterFedFieldValue) != 'undefined'&& matterFedFieldValue != ''){
                            missingFieldList += itemcmp.get("v.label").split('Federal ID')[0]+'\n';
                        }
                    }
                }
        ...
        console.log('missingFieldList:'+missingFieldList);
        return missingFieldList;
    },
})