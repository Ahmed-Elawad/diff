 ...
 */
trigger RefCorePayUpdateCase on Reference_Core_Payroll__c (after update) {

   /* The records to be updated. */
   Map<Id,String> caseStepMap = new Map<Id,String>();
   for ( Reference_Core_Payroll__c newRT: Trigger.new) {
   	  System.debug('RefCorePayUpdateCase checking record newRT.Current_Step__c='+newRT.Current_Step__c);
      ...
      if ((newRT.Case_Lookup__c != null) && (newRT.Current_Step__c != '')) {
         if (Trigger.isUpdate) {
            ...
            Reference_Core_Payroll__c oldRT = Trigger.oldMap.get(newRT.id);
